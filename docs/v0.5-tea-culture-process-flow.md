...
第一轮的五步流程只解决了“知道顺序”，不足以呈现制茶技艺的判断、地方差异和日常社交语境。第二轮在不增加未经授权媒体的前提下，补充四个数据驱动的互动层：

| 互动层 | 用户动作 | 得到的信息 | 内容约束 |
| --- | --- | --- | --- |
| 茶山地图 | 选择一个地方实践 | 产地、茶类与该实践的工序观察点 | 使用示意关系图，不暗示精确地理边界 |
| 制茶实验台 | 在绿茶、乌龙茶、白茶三种观察路径间切换 | 同一鲜叶在不同工序重点中的变化 | 说明是教育性简化，不能当作制作配方 |
| 茶席任务 | 选择家庭待客、茶馆相聚或节日团聚场景 | 茶如何在不同场景中连接人与人 | 不设“唯一正确”的礼仪判断 |
| 器物观察 | 选择盖碗、壶、杯、茶盘 | 器物与注水、分饮、承接等动作的关系 | 图文降级优先，保留后续 360° 素材接口 |

新增字段 `makingPaths`、`socialContexts` 和 `teaWare`。它们均属于展厅内容，不纳入 UNESCO 名录字段；展示时须保留“教育性观察”标签与来源区块。
...
7. 三种制茶观察路径、三种茶席场景与四件茶器均可切换；选择不会只依赖颜色表达状态。
