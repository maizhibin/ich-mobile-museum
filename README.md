...
- v0.5.4：京剧馆与茶文化馆改用 CosyVoice 3 神经网络合成语音，按馆别调校语气和停顿，并补齐逐句同步时间轴。
- v0.5.5：导览改为可独立重做的逐句音频与整段进度播放；京剧后台发布图改用高清压缩 WebP，原始 PNG 转入源素材目录。
- v0.5.6：京剧馆新增四幕 AI 概念插画跑马灯；茶文化馆加入山间茶室球面全景、三处空间讲解点、陀螺仪与图文降级；两馆复用同一套可配置全景组件。
...
- 生、旦、净、丑四大行当介绍。
- 唱、念、做、打基本功展示。
- 带中文合成语音、章节跳转、完整文稿逐句高亮与点击跳转、整段进度和倍速的连续导览播放器；底层音频按句存储，可独立重做。
- 导览提供完整文稿、更新时间、官方来源和合成语音标识，不冒充真人馆员或传承人录音。
- 可运行 `pnpm audio:generate`，使用 CosyVoice 3 重新生成两馆导览及同步时间轴；模型、环境与披露要求见 [`docs/cosyvoice-audio-guide.md`](docs/cosyvoice-audio-guide.md)。
- 新展馆可使用内网 GPU CosyVoice 3 服务生成逐句音频并下载为项目包；导入流程、AI 语音披露与维护方式见 [`docs/cosyvoice3-remote-service.md`](docs/cosyvoice3-remote-service.md)。
- 京剧、昆曲、粤剧、越剧横向比较。
...
- 采茶、制茶、泡茶、茶礼与茶器流程，以及五类地方实践比较路径。
- 带同步文字稿、章节和合成语音标识的三分钟导览。
- 支持触控、键盘、移动端陀螺仪和空间讲解点的山间茶室球面全景。
- 全景使用 AI 概念图并明确标注，保留等价图文降级。
...
- 操作京剧播放器、图片跑马灯、剧种比较台，以及京剧与茶文化馆的 WebGL 球面全景和图文降级模式。
...
2. 格式、lint、类型和单元测试检查。
3. Vite 生产构建。
4. 校验逐句音频、试听音频和压缩图片均已进入 `dist/`，且高清源图未进入发布产物。
